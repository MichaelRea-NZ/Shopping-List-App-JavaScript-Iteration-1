...
class View {
  ...
  showGroceryList(pantryList){
	  ...
	  if (pantryList.length ===0){
		  const introPlaceholder = this.createElement('introPlaceholder')
		  introPlaceholder.textContent = 'Add a groceries to start list'
		  this.groceryList.append(introPlaceholder)
	  }else{
		  
		  pantryList.forEach(groceryItem =>{
			const itemLine = this.createElement('li')
			itemLine.id = groceryItem.id	
        ...
        checkbox.checked = groceryItem.complete					
          ...
          itemLine.append(checkbox, span, deleteButton)
		  
		  
		  this.groceryList.append(itemLine)
		 })
	}
   ...
   bindEditItem(handler){
	   this.groceryList.addEventListener('focusout', event => {
		   if(this.viewTempGroceryText){
			   ...
			   handler(id, this.viewTempGroceryText())
			   this.viewTempGroceryText = ''
		   }
	   })
   }
